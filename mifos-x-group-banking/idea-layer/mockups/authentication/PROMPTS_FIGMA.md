# Authentication — Figma Prompts (Auto-Complete Style)

> Generated from v3.1 YAML siblings via /idea-migration reference.
> Short Figma-optimized per-state prompts for CommonPurse (authentication).
...
- Feature: Authentication · CommonPurse banking app · 8 components · 4 states
...
Client type selection + Fineract credential login + admin landing
...
Screen: client-type-selector · State: selected. Scrollable column, Material3, primary #2E7D32. TopAppBar + content sections + FAB where applicable. 48dp min touch targets. Noto Sans typography.
...
Screen: login · State: biometric. Scrollable column, Material3, primary #2E7D32. TopAppBar + content sections + FAB where applicable. 48dp min touch targets. Noto Sans typography.
...
Dialog overlay on dimmed background. Card (360dp max-width, 16dp radius, elevation 3). Title (titleLarge), body text (bodyMedium), Cancel (text button) + Confirm (filled tonal, errorContainer) button row.

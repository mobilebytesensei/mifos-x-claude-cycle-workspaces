...
Sync queue status, pending operations, conflict resolution
...
Same as offline but chip shows sync icon with rotation animation. 'Sync Now' button replaced by progress indicator. Operations transitioning to in_progress show indeterminate progress.
### State: `synced`

Same layout but chip shows cloud_done (green). All operations show completed checkmarks. 'All caught up' empty state after list clears.
### State: `conflict`

Operations with conflict status highlighted in amber row background. Conflict icon + description. 'Resolve' action per conflicted operation. Admin contact guidance at bottom.
### State: `error`

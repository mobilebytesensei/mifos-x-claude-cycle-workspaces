...
| top_bar `more_vert` | OnMoreOptions | transform_state | Expands `group-dashboard-more-menu` Compose-DropdownMenu; local UI state only |
...
| view_savings_button | OnViewSavings | navigate | NavigateToMemberSavingsDetail(groupId); MEMBER only; personal savings ledger from companion accounts endpoint |

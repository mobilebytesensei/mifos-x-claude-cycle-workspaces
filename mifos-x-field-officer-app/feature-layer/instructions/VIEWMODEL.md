...
# template_path: "workspaces/mifos-x/mifos-x-field-officer-app/feature-layer/instructions/VIEWMODEL.md"
...
All types are co-located in a single `{Feature}ViewModel.kt` file:

```kotlin
// [Feature]ViewModel.kt - ViewModel + State/Event/Action (co-located)
...
internal class [Feature]ViewModel(
    ...
    private fun handleDeleteResult(result: DataState<Unit>) {
        when (result) {
            ...
            is DataState.Error -> {
                updateState {
                    it.copy(dialogState = [Feature]State.DialogState.Error(result.error.message ?: "Unknown error"))
                }
            }
        }
    }
